module Peers
  class ConnectionsManagementController < ApplicationController
    def index
    ...
    def create
      ...
      if connection.persisted? && connection.pending?
        redirect_to peers_connections_path, notice: "Connection request sent to #{connection.hostname}"
      else
    ...
    def accept
      ...
      unless connection
        return redirect_to peers_connections_path, alert: "Connection not found"
      end

      unless connection.pending?
        return redirect_to peers_connections_path, alert: "Connection is not pending"
      end
      ...
      if connection.active?
        ...
        redirect_to peers_connections_path, alert: "Could not complete connection: #{connection.error_message || 'Unknown error'}"
      end
    ...
    def reject
      ...
      connection.reject!
      redirect_to peers_connections_path, notice: "Connection request from #{hostname} rejected"
    end
